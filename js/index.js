...
output.innerHTML += `  <div class=" proj-card"> <div class="img-container"><img src='${project.img.trim()}'alt='${project.img}' class="mobile-view"><img src='${project.img.trim()}'alt='${project.img}'class="desktop-view"></div><div class="prof-title"><h3>${project.name}</h3><div class="proj-status"><label>In Progress</label></div></div><p>${project.intro}</p><span>${project.techStock}</span>      <div class="likeView"><button class="btnLike" data-btnprog_name='${project.name}'><i class="fa fa-thumbs-up"></i><p class="likeVal" id="like-${project.name}">0</p></button><button ><i class="fa fa-eye"></i><p id="numbView">0</p></button></div>
      <a href='${project.link}' class="play-btn">Review Document</a></div>`;  
});
